...
- `NotroContent` renders article bodies, with `buildLinkToPages()` resolving references to published articles. The existing IVS CSS controls presentation. The sidebar reads the rendered Heading 2 IDs in the browser, so its anchors match Notro's handling of duplicate headings. Article content remains available without JavaScript.
